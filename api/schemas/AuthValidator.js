...
module.exports = class AuthValidator{
    ...
    signup(){
        ...
        let uname = Joi.alternatives().try( Joi.string().regex(/^\w{3,30}$/), Joi.string().regex(/^[-!#$%&'*+\/0-9=?A-Z^_a-z{|}~](\.?[-!#$%&'*+\/0-9=?A-Z^_a-z`{|}~])*@[a-zA-Z0-9](-?\.?[a-zA-Z0-9])*\.[a-zA-Z](-?[a-zA-Z0-9])+$/)).required()
        let emailsetup = {
        ...
        return Joi.object().keys({
            username: uname,
            password:Joi.string().regex(/^[^\s]{4,30}$/).required(),
