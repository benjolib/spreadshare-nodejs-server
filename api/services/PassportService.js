...
module.exports = class PassportService extends Service {
  ...
  init() {
    ...
    let {
      ...
      google,
      twitter
    } = this.app.config.passport.strategies;

    passport.use(
      new basic.strategy((userid, passwd, done) => {
        User.findOne({
          where: { email: userid },
    ...
    passport.use(
      new jwt.strategy(jwt.options, function(jwt_payload, done) {
        User.findOne({
